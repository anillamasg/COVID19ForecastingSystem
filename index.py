...
def compare_prediction(location1, location2, metric):
    ...
    predictedCases2 = [1153, 1153, 1155, 1155, 1156, 1158, 1158, 1158, 1158, 1158] #confirmed_QLD_cases

    fig = make_subplots()
    fig.add_trace(
        go.Scatter(x=dateList, y=casesList1, name="Total " + location1 + " " + metricDictionary[metric],
                   marker=dict(color='#0400FF')))
    fig.add_trace(go.Scatter(x=predictedDate, y=predictedCases1, name="Predicted " + location1 + " " + metricDictionary[metric],
                             marker=dict(color='#EB752C')))
    fig.add_trace(
        go.Scatter(x=dateList, y=casesList2, name="Total " + location2 + " " + metricDictionary[metric],
                   marker=dict(color='#F600FA')))
    fig.add_trace(go.Scatter(x=predictedDate, y=predictedCases2, name="Predicted " + location2 + " " + metricDictionary[metric],
    ...
    if location1 < location2:
        fig.write_html("plots/views/compare/" + metricDictionary[metric].lower().replace(" ", "_") + "/" + location1 + "_" + location2 + "_comparison" + ".html")
    else:
        fig.write_html("plots/views/compare/" + metricDictionary[metric].lower().replace(" ", "_") + "/" + location2 + "_" + location1 + "_comparison" + ".html")


compare_prediction('NSW', 'QLD', 'confirmed')
